  ...
  on('task', {
    ...
    async setupMetamask({ secretWords, network, password }) {
      if (puppeteer.metamaskWindow()) {
        ...
        if(process.env.NAKA_TOKEN_CONTRACT){
          token_contract_address = process.env.NAKA_TOKEN_CONTRACT
        }
        await metamask.initialSetup({ secretWords, network, password,token_contract_address });
        return true;
      }
    },
