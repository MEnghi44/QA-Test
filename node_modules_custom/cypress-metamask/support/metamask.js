...
module.exports = {
  ...
  async addToken(token_contract_address) {
    return new Promise(async (resolve, reject) => {
      try {
        await puppeteer.metamaskWindow().waitForFunction(`document.querySelector(".import-token-link__link").innerText !=""`)
        await puppeteer.metamaskWindow().evaluate(async () => {
          var link = document.querySelector(".import-token-link__link")
          if (link) {
            link.click()
            return true
          } else {
        ...
        await puppeteer.waitAndType("#custom-address", token_contract_address)
        await puppeteer.metamaskWindow().waitForSelector('footer > .page-container__footer-button:not([disabled])');
        await puppeteer.metamaskWindow().waitForFunction('document.querySelector("#custom-decimals").value != "0"');
        await puppeteer.waitAndClick(".page-container__footer-button")
        await puppeteer.metamaskWindow().waitForFunction('document.querySelector(".confirm-import-token__name").innerText != ""')
        await puppeteer.waitAndClick(".page-container__footer-button:nth-child(2)")
        resolve(true)
      } catch (error) {
        resolve(error)
      }
  ...
  async closeTabMateMask() {
    return await puppeteer.metamaskWindow().close();
  },
  async initialSetup({ secretWords, network, password, token_contract_address }) {
    ...
    await puppeteer.metamaskWindow()
      .waitForTimeout(5000);
    ...
    if (
      ...
      if (isCustomNetwork) {
        ...
        if (token_contract_address) {
          await module.exports.addToken(token_contract_address)
        }
      } else {
        await module.exports.changeNetwork(network);
      }
      //  await module.exports.getWalletAddress();

      return await puppeteer.mainWindow().bringToFront();
    } else {
